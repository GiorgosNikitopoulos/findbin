...
def main(args):
    ...
    for tar_file in glob.glob(f"{args.input_path}/*"):
        print(tar_file)
        #Untar file in temp
        result = subprocess.run(["tar", "-xf", tar_file, "-C", f"{args.temp_dir}"], capture_output=True, text=True)

        #Depackage all the debs out of the directory to temp
        for deb_file in glob.glob("/temp/**/*.deb", recursive = True):
        ...
        for each_file in glob.glob('/temp/**', recursive = True):
        ...
        #Clean up directory 
        subprocess.run(f'rm -rf {args.temp_dir}/*', shell=True)
